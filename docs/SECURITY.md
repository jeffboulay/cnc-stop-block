...
| **Solution** | CORS origin is now configurable via the `CORS_ORIGIN` build flag (e.g. `-DCORS_ORIGIN=\"http://192.168.1.50:5173\"`). Defaults to `*` for development convenience; set it in `platformio.ini` build_flags for production |
...
| **Solution** | WebSocket connections capped at `WS_MAX_CLIENTS` (4, defined in `config.h`). Excess connections are closed on connect. Per-IP rate limiting deferred to next steps |
