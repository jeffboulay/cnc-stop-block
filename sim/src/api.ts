...
export function buildRouter(sim: SimMachine): Router {
  ...
  router.post('/api/goto', (req, res) => {
    ...
    if (!isValidFloat(pos) || pos < 0 || pos > MAX_TRAVEL_MM) {
      res.status(400).json({ error: `position_mm out of range (0 - ${MAX_TRAVEL_MM})` })
      return
    }
    sim.commandGoTo(pos)
    res.json({ ok: true })
  ...
  router.post('/api/cutlist/add', (req, res) => {
    ...
    if (!isValidFloat(lengthMM) || lengthMM <= 0 || lengthMM > MAX_TRAVEL_MM) {
      res.status(400).json({ error: `length_mm out of range (0 - ${MAX_TRAVEL_MM})` })
      return
    }
  ...
  router.post('/api/cutlist/clear', (req, res) => {
    if (sim.state === 'CUTTING') {
      res.status(409).json({ error: 'Cannot modify cut list during active cut' })
      return
    }
    sim.clearCuts()
    res.json({ ok: true })
  ...
  router.post('/api/tools', (req, res) => {
    ...
    if (typeof uid !== 'string' || uid.length === 0 || uid.length > MAX_STRING_LENGTH) {
      res.status(400).json({ error: `uid must be 1-${MAX_STRING_LENGTH} characters` })
      return
    }
    ...
    const tool: ToolInfo = { uid, name: truncate(name, MAX_STRING_LENGTH), kerf_mm: kerfMM }
  ...
  router.post('/sim/inject/position', (req, res) => {
    ...
    if (!isValidFloat(pos) || pos < 0 || pos > MAX_TRAVEL_MM) {
      res.status(400).json({ error: `pos_mm out of range (0 - ${MAX_TRAVEL_MM})` })
      return
    }
    sim.positionMM = pos
